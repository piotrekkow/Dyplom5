#include "entry_item.hpp"

#include <qpainter.h>

#include <QPainter>

EntryItem::EntryItem(EntryId id, QPointF origin, QPointF heading)
    : id_(id), heading_(heading) {
    setPos(origin);
    recomputeBounds();
}

QPainterPath EntryItem::shape() const {
    QPainterPath p;
...
void EntryItem::setLanePoints(const std::vector<QPointF>& scenePoints) {
